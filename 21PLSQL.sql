...
DECLARE
    DEPT_ID NUMBER;
    SUMS NUMBER;
    CURSOR X IS SELECT DEPARTMENT_ID, SUM(SALARY) AS SUMS FROM EMPLOYEES GROUP BY DEPARTMENT_ID ORDER BY DEPARTMENT_ID DESC;
BEGIN
    OPEN X;
    LOOP
        FETCH X INTO DEPT_ID, SUMS;
        EXIT WHEN X%NOTFOUND;
        DBMS_OUTPUT.PUT_LINE('부서:' || DEPT_ID || ' 급여:' || SUMS);
    END LOOP;
    CLOSE X;
END;
