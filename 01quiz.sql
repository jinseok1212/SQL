...
SELECT EMPLOYEE_ID AS 사원번호, FIRST_NAME || LAST_NAME AS 사원명, HIRE_DATE AS 입사일자, TRUNC((SYSDATE - HIRE_DATE) / 365) AS 근속년수 FROM EMPLOYEES WHERE TRUNC((SYSDATE - HIRE_DATE) / 365) > 10 ORDER BY 근속년수 DESC ;
--문제 2.
...
, TO_CHAR(SALARY + SALARY * NVL(COMMISSION_PCT, 0)) AS 달러
, TO_CHAR(NVL2(COMMISSION_PCT, SALARY + SALARY * COMMISSION_PCT, SALARY)*1300, 'L999,999,999,999') AS 급여
, DECODE(MOD(TRUNC((SYSDATE - HIRE_DATE) / 365),5), 0, '진급대상') AS 진급대상 FROM EMPLOYEES WHERE DEPARTMENT_ID IS NOT NULL;