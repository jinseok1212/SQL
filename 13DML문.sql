...
--DML문은 트랜잭션이 항상 기록됨 ROLLBACK 이용해서 되돌릴 수 있음
ROLLBACK;
...
--------------------------------------------------------------------------------
--DELETE 구문
--트랜잭션이 있긴 하지만 삭제하기 전에 반드시 SELECT 문으로 삭제 조건에 해당하는 데이터를 꼭 확인하는 습관을 들이자
SELECT * FROM EMPS WHERE EMPLOYEE_ID = 148;
DELETE FROM EMPS WHERE EMPLOYEE_ID = 148; --KEY를 통해서 지우는 편이 좋음
SELECT * FROM EMPS;

--DELECT 구문도 서브쿼리
DELETE FROM EMPS WHERE EMPLOYEE_ID IN (SELECT EMPLOYEE_ID FROM EMPLOYEES WHERE DEPARTMENT_ID = 80);--다중 행임
ROLLBACK;
---------------------------------------------------------------------
...
SELECT * FROM DEPARTMENTS;
DELETE FROM DEPARTMENTS WHERE DEPARTMENT_ID = 100; --EMPLOYEES에서 100번 데이터를 FK로 사용하기에 안지워짐
----------------------------------------------------------------------
...
SELECT * FROM EMPS;
--2ND - 서브쿼리절로 다른 테이블을 가져오는 게 아니라 직접 값을 넣을 때 DUAL을 쓸 수도 있음
MERGE INTO EMPS A
USING DUAL
