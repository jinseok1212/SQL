...
SELECT FIRST_NAME, INSTR(FIRST_NAME, 'A') FROM EMPLOYEES; -- A가 있는 위치를 반환함, 없는 경우는 0 을 반환
...
SELECT RPAD(SUBSTR(LOWER(FIRST_NAME), 1, 3 ), LENGTH(FIRST_NAME), '*') AS NAME, LPAD(SALARY, 10, '*') FROM EMPLOYEES WHERE LOWER(JOB_ID) = 'it_prog';